      ...
      <div className="flex-1 flex flex-col overflow-y-auto p-8 relative">
        ...
        <div className="flex justify-between items-center mb-8 border-b border-slate-200 dark:border-slate-900 pb-4">
          <div className="space-y-1">
            <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-50">إدارة فروع الأكاديمية</h1>
            <p className="text-sm text-slate-600 dark:text-slate-400">اضبط الفروع والمقرات الفعلية التابعة لأكاديمية {tenant?.name}.</p>
          </div>
          <Link
            to="/dashboard"
            className="text-xs font-semibold text-violet-700 dark:text-violet-400 hover:text-violet-700 dark:hover:text-violet-300"
          >
            ← العودة للوحة التحكم
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start max-w-6xl">
          ...
          <div className="lg:col-span-2 space-y-4">
            <h2 className="text-lg font-bold text-slate-700 dark:text-slate-300 mb-2">الفروع الحالية</h2>
            ...
            {loading ? (
              ...
              <div className="rounded-xl border border-slate-200 dark:border-slate-900 border-dashed p-12 text-center text-slate-500">
                <GitBranch className="h-10 w-10 mx-auto mb-4 text-slate-600 dark:text-slate-300" />
                <p className="text-sm font-semibold">لا توجد فروع مضافة حالياً.</p>
                <p className="text-xs text-slate-600 dark:text-slate-300 mt-1">استخدم النموذج الجانبي لإضافة أول فرع لمركزك.</p>
              </div>
            ) : (
                ...
                {branches.map((branch) => (
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="space-y-1">
                        <h3 className="font-bold text-slate-800 dark:text-slate-200">{branch.name}</h3>
                        {branch.address && (
                      ...
                      <button
                        ...
                        className="h-8 w-8 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-red-950 hover:bg-red-100 dark:hover:bg-red-950/10 text-slate-500 hover:text-red-700 dark:hover:text-red-400 flex items-center justify-center transition-all cursor-pointer opacity-0 group-hover:opacity-100"
                      >
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 px-4 py-2.5 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-600 outline-none focus:border-violet-200 dark:focus:border-violet-500/50 focus:ring-2 focus:ring-violet-500/20 transition-all text-right"
                />
              </div>
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-lg bg-violet-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-violet-500 disabled:bg-violet-800 disabled:text-slate-600 dark:text-slate-400 transition-all cursor-pointer shadow-lg shadow-violet-600/10"
              >
                {isSubmitting ? (
                  <>
