...
export const AcademicHub: React.FC = () => {
  ...
  const [sessionFilter, setSessionFilter] = useState<'all' | 'daily' | 'weekly' | 'monthly'>('all')
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                ...
                {groups.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {groups.map((g) => (
                      <div key={g.id} className="rounded-xl border border-slate-200 dark:border-slate-900 bg-slate-50 dark:bg-slate-950/40 p-5 hover:border-slate-200 dark:border-slate-800 transition-all space-y-3">
                        <div>
              ...
              (() => {
                const filtered = sessions.filter(s => {
                  ...
                  const sTime = sDate.setHours(0, 0, 0, 0)
                  const today = new Date()
                  const todayTime = today.setHours(0, 0, 0, 0)
                  
                  if (sessionFilter === 'daily') {
                    return sTime === todayTime
                  }
                  
                  if (sessionFilter === 'weekly') {
                    ...
                    return sTime >= startOfWeek.getTime() && sTime <= endOfWeek.getTime()
                  }
                  
                  if (sessionFilter === 'monthly') {
                ...
                return (
                  <div className="space-y-4">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-slate-200 dark:border-slate-800 pb-3">
                      ...
                      <div className="flex bg-slate-100 dark:bg-slate-900 p-1 rounded-lg border border-slate-200 dark:border-slate-800" dir="rtl">
                        {(['all', 'daily', 'weekly', 'monthly'] as const).map((filter) => (
                          ...
                          >
                            {filter === 'all' ? 'الكل' : filter === 'daily' ? 'يومي' : filter === 'weekly' ? 'أسبوعي' : 'شهري'}
                          </button>
                        ))}
                      </div>
                    </div>
                    ...
                    {filtered.length === 0 ? (
                      <div className="rounded-xl border border-slate-200 dark:border-slate-900 border-dashed p-12 text-center text-slate-500">
                        ...
                        <p className="text-sm font-semibold">لا توجد حصص مجدولة لهذه الفترة.</p>
                      </div>
                    ) : (
